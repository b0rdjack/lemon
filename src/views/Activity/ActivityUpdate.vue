<template>
  <div class="new-activity">
    ...
    <div v-else>
      <b-row class="new-activity-row mt-5" v-if="form.state.id === 3">
        <b-col class="ml-2">
    ...
    <b-row class="mt-5 text-center buttons" v-if="form.state.id === 3">
      <b-col>
        <b-button
          size="lg"
    ...
    >
      <b-row>
        <b-col class="text-center">
          <h3>Mise à jour de l'activité</h3>
        </b-col>
      </b-row>
      <b-row>
        <b-col></b-col>
...
export default {
  ...
  methods: {
      ...
      await fetch(API_URL + "activities/" + this.$route.params.id, {
        ...
        .then(async (response) => {
          ...
          if (response.error) {
            ...
            this.form.opening_hours = this.getHMS(this.form.opening_hours);
            this.form.closing_hours = this.getHMS(this.form.closing_hours);
            this.form.average_time_spent = this.getHMS(
              this.form.average_time_spent
            );
            let tmp_tags = [];
            this.form.tags.forEach(element => {
              tmp_tags.push(element.id);
            });
            this.form.tags = tmp_tags;
          }
        })
        .catch((error) => {
          console.error(error);
    ...
    async onSubmit(e) {
      ...
      fetch(API_URL + "activities/" + this.$route.params.id, {
        method: "PUT",
        headers: {
          Accept: "application/json",
          "Content-Type": "application/json",
